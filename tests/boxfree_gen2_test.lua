...
-- (screen.lua, "where in the grid each POKeMON sits").
...
local Screen = assert(load(assert(slurp("gen2screen.lua")),
  "@gen2screen.lua"))()(mod)
...
-- ---- the icon rule is this mod's own, not the bundle's
--
-- `iconFor` picks the frame off one clock, so every icon drawn through it
-- flips together.  The rule that rests an unhovered one on frame 0 used to
-- live only in the Gen1Wild bundle's runtime/icons2.lua -- so on a standalone
-- install nothing honoured `gen1wildAnimate` and every cell in the box flipped
-- at once, which is the thing this screen exists to stop.  Nothing in this
-- file loads a bundle runtime, which is the point: what follows is what a
-- player gets with this mod and nothing else.
do
  local screen = newScreen({ "A", "B", "C", "D" })
  local IMG = { getDimensions = function() return 16, 32 end }
  PartyMenu.iconIdFor = function() return "ICON_X" end
  screen.icons.icons = { icons = { ICON_X = { image = "p.png" } } }
  package.loaded["src.render.Assets"].image = function() return IMG end
  ...
  screen.icons.clock = 16          -- an animating icon is on frame 1 here
  screen.icons.gen1wildAnimate = true
  local _, hovered = screen.icons:iconFor({})
  eq(hovered, 1, "the icon under the cursor walks")
  screen.icons.gen1wildAnimate = false
  local _, still = screen.icons:iconFor({})
  eq(still, 0, "and every other one rests on frame 0, with no bundle installed")
end
...
do
  ...
  local gen1 = assert(slurp("screen.lua"))
