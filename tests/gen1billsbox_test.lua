...
-- ------- the POKeMON screen wears the same colours
--
-- The box's rule is one species palette per POKeMON.  The party menu draws
-- the same six POKeMON in the same 16x16 icons, one screen away, and the cart
-- put a single MEWMON block over the whole column -- so the two screens
-- disagreed about what colour a POKeMON was.
--
-- What is asserted here is the whole of the change: the engine's own screen,
-- decorated rather than replaced, keeping its base and its HP bar blocks and
-- gaining one zone per row; a full-colour icon marked instead of coloured;
-- and the option putting the engine's own answer back untouched.
...
do
  ...
  local partyFactory = Data.screens and Data.screens.PartyMenu
  T.check(partyFactory ~= nil, "the party menu screen id is taken over too")
  T.eq(type(partyFactory.new), "function", "and it is a screen factory")

  Data.icons = {
    -- a mod's own image, and this one is colour: it must be marked, not zoned
    bySpecies = { FIXMON_A = { image = "mods/pack/vivid_a.png" },
                  -- a built-in icon CLASS, baked grey through obpIcon
                  FIXMON_B = "MON" },
    icons = { MON = "assets/icons/mon.png" },
    byDex = {},
  }
  Data.palettes = {
    palettes = { MEWMON = { WHITE, { 239, 156, 107 }, { 115, 33, 165 }, BLACK },
                 GREENMON = { WHITE, { 99, 255, 90 }, { 255, 99, 140 }, BLACK },
                 GREENBAR = { WHITE, { 0, 255, 0 }, { 0, 168, 0 }, BLACK },
                 YELLOWBAR = { WHITE, { 255, 255, 0 }, { 168, 168, 0 }, BLACK },
                 REDBAR = { WHITE, { 255, 0, 0 }, { 168, 0, 0 }, BLACK } },
    pokemon = { FIXMON_A = "GREENMON", FIXMON_B = "GREENMON" },
  }
  ...
  love.image.newImageData = function(a, ...)
    local name = type(a) == "table" and (a.name or a.path) or tostring(a)
    return setmetatable({ w = 16, h = 16,
                          colour = tostring(name):find("vivid", 1, true) ~= nil },
                        FakeData)
  end

  local function member(species)
    return { species = species, level = 5, nickname = species,
             hp = 20, stats = { hp = 20 } }
  end

  local vivid, plain = member("FIXMON_A"), member("FIXMON_B")
  local game = fakeGame({}, { vivid, plain })
  local screen = partyFactory.new(game, {})

  -- decorated, not replaced: everything the engine's party menu does -- the
  -- switching, the field moves, the item targeting -- is still its own
  T.eq(getmetatable(screen), PartyMenu,
    "the screen IS the engine's party menu, with two methods decorated onto it")

  local function byRect(zones)
    local out = {}
    for _, z in ipairs(zones or {}) do
      out[z.x .. "," .. z.y .. "," .. z.w .. "," .. z.h] = z
    end
    return out
  end

  local zones = screen:sgbPalettes(game)
  local rects = byRect(zones)

  -- what the cart drew is still there: the GREENBAR base, the MEWMON block
  -- over the icon column, and one bar block per row
  T.check(rects["0,0,160,144"] ~= nil, "the base palette still covers the screen")
  T.check(rects["8,0,16,96"] ~= nil,
    "and the cart's MEWMON block is still under the icon column")
  T.check(rects["48,8,56,8"] ~= nil, "with the HP bar rows keeping their own")

  -- and on top of it, one species zone per row: two tiles square, over the
  -- icon, in that POKeMON's own colours
  local second = rects["8,16,16,16"]
  T.check(second ~= nil, "an ordinary DMG icon gets its species zone")
  T.same(second.colors, Data.palettes.palettes.GREENMON,
    "carrying that species' palette")
  T.eq(rects["8,0,16,16"], nil,
    "a full-colour icon gets none -- it would be paint nobody sees")

  -- the zone sits ON the icon: PartyMenu.drawIcon is called at x = 8 and the
  -- engine's own entryY, and the zone is that rect to the pixel
  do
    local calls = {}
    local realIcon = PartyMenu.drawIcon
    PartyMenu.drawIcon = function(_, m, x, y)
      calls[#calls + 1] = { mon = m, x = x, y = y }
    end
    local ok = pcall(function() screen:draw() end)
    PartyMenu.drawIcon = realIcon
    T.check(ok, "the party menu still draws")
    T.eq(#calls, 2, "one icon per party member")
    T.eq(calls[2].x .. "," .. calls[2].y, "8,16",
      "the second row's icon is where its zone is")
    T.eq(PartyMenu.entryY(2), 16, "which is the engine's own row geometry")
  end

  -- the full-colour one is claimed instead, which takes it out of the pass
  local marks = {}
  local realMark = P.markTrueColor
  P.markTrueColor = function(x, y, w, h)
    marks[#marks + 1] = x .. "," .. y .. "," .. w .. "," .. h
  ...
  P.markTrueColor = realMark
  T.eq(#marks, 1, "exactly one region is claimed as full colour")
  T.eq(marks[1], "8,0,16,16", "the vivid icon's own rect, on its own row")

  -- a battle's local party view is what gets coloured, not save.party
  do
    local borrowed = member("FIXMON_B")
    local scoped = partyFactory.new(game, { party = { borrowed } })
    local only = byRect(scoped:sgbPalettes(game))
    T.check(only["8,0,16,16"] ~= nil,
      "the party the screen was handed is the party that is coloured")
    T.eq(only["8,16,16,16"], nil, "and nothing is coloured under an empty row")
  end

  -- PARTY COLOURS off puts the engine's own answer back, untouched
  do
    local stored = run.loader.modOptions
    local id = run.mod.manifest.id
    local was = stored[id]
    stored[id] = { partyColours = false }

    local off = byRect(partyFactory.new(game, {}):sgbPalettes(game))
    T.check(off["8,0,16,96"] ~= nil, "the MEWMON block is still there")
    T.eq(off["8,16,16,16"], nil, "with no species zone over it")

    marks = {}
    P.markTrueColor = function(x, y, w, h)
      marks[#marks + 1] = x .. "," .. y .. "," .. w .. "," .. h
    end
    pcall(function() partyFactory.new(game, {}):draw() end)
    P.markTrueColor = realMark
    T.eq(#marks, 0, "and nothing claimed: the screen is the engine's again")
