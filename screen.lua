...
return function(mod, Icons)
  ...
  function Screen:sgbPalettes(game)
    local ok, zones = pcall(function()
      ...
      local function paint(mon, tx1, ty1, tx2, ty2)
        ...
        if Icons.fullColourRect(game, mon) then return end
  ...
  -- The party menu's own icon renderer, with the full-colour mark that has to
  -- go with it -- both in iconart.lua, because the party screen needs the same
  -- pair.
  function Screen:drawIcon(mon, x, y, selected)
    Icons.draw(self.game, mon, x, y, selected and self:animAlt() or false)
  end
